...
import { getWeekNumber, getWeekDates, generateId } from '@/lib/utils'
...
export default function NewReportPage() {
  ...
  async function handleCreate() {
    ...
    const report: WeeklyReport = {
      ...
      year,
      week,
      title: title || `${year} 年第 ${week} 週家庭週報`,
      startDate: start.toISOString(),
      endDate: end.toISOString(),
      entries: MEMBERS.map(member => ({ member, reflection: '', updatedAt: new Date().toISOString() })),
      photos: [],
      createdAt: new Date().toISOString(),
    }
    await saveReport(report)
    router.push(`/report?id=${report.id}&edit=true`)
  }
