...
import { uploadPhoto, deletePhoto, getReport, saveReport } from '@/lib/storage'
...
export function PhotoGallery({ photos, reportId, editable, onPhotosChange }: Props) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await saveReport({ ...report, photos: updated })
    ...
    <div className="space-y-4">
      {editable && (
        <div className="flex flex-wrap items-center gap-3 p-4 bg-gray-50 rounded-xl border border-dashed border-gray-300">
          <select value={selectedMember} onChange={e => setSelectedMember(e.target.value as Member)}
          ...
          <span className="text-xs text-gray-500">可同時選取多張</span>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {photos.map(photo => (
            <div key={photo.id} className="group relative aspect-square rounded-xl overflow-hidden bg-gray-100 shadow-sm">
              <img src={photo.url} alt={photo.caption || '家庭照片'}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
          <div className="relative max-w-3xl max-h-full" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setLightbox(null)}
              className="absolute top-3 right-3 w-8 h-8 bg-white/20 hover:bg-white/40 text-white rounded-full flex items-center justify-center">✕</button>
          </div>
        </div>
      )}
    </div>
